...
// Test that the onmozbrowsermetachange event for urlbar-control works.
"use strict";
...
function runTest() {
  ...
  iframe1.src = "http://test/tests/dom/browser-element/mochitest/file_browserElement_UrlbarControlMeta.html";
  ...
  function tests(e) {
    ...
    switch (numMetaChanges++) {
      case 0:
        {
          is(detail.name, 'urlbar-control', 'name matches');
          is(detail.content, 'minimized', 'content matches');
          is(detail.type, 'added', 'type matches');

          let script =
            "var meta = content.document.head.querySelector('meta');" +
            "meta.content = 'maximized';";
      ...
      case 1:
        {
          is(detail.name, 'urlbar-control', 'name matches');
          is(detail.content, 'maximized', 'content matches');
